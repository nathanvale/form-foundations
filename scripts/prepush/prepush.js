...
async function resetSoft(branchName) {
  await execGitCmdFeedback('reset', '--soft', 'main');
}
...
async function mergeMain({ stashed, branchName }) {
  console.log('\r\n');
  console.log(
    chalk.italic(`Attempting to merge the main branch into ${branchName}:`)
  );
  console.log('\r');
  let result;
  ...
  await execGitCmdFeedback('checkout', 'main').catch(revertChanges);
  await execGitCmdFeedback('pull').catch(revertChanges);
  await execGitCmdFeedback('checkout', '--force', branchName);
  if (stashed) {
    await execGitCmdFeedback('stash', 'apply');
  }
  await execGitCmdFeedback('merge', 'main');
}
...
async function runPrepush() {
  ...
  await mergeMain({ branchName, stashed }).catch(e => {
    throw new Error(
      e +
        '\r\n' +
        chalk.reset.inverse.bold.blue(' Tip ') +
